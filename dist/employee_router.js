...
function getEmployeeByID(id) {
    let employee = employeeList.find(data => data.id == id);
    return employee;
}
...
function patchEmployeeByID(id, params) {
    let employee = null;
    for (let i = 0; i < employeeList.length; i++) {
        if (employeeList[i].id == id) {
            if ("name" in params)
                employeeList[i].name = params.name;
            if ("phone" in params)
                employeeList[i].phone = params.phone;
            if ("mail" in params)
                employeeList[i].mail = params.mail;
            employeeList[i].updatedAt = new Date();
            employee = employeeList[i];
        }
    }
